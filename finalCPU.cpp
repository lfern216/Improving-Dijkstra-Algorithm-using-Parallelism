...
void dijkstra(long* matrix, int src) {
	...
	int dist[V];     // The output array.  dist[i] will hold the shortest
     ...
     for (int count = 0; count < V-1; count++)
     {
       ...
       for (int k = 0; k < V; k++)
         ...
         if (!sptSet[k] && matrix[u*V+k] && dist[u] != INT_MAX 
                                       && dist[u]+matrix[u*V+k] < dist[k])
            dist[k] = dist[u] + matrix[u*V+k];
     }
...
double elapse = (end.tv_usec - start.tv_usec);
